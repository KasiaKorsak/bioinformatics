function [ m,all,q ] = local( str1,str2,match,mismatch,gap )
...
for i=2:a %wiersze
   for j=2:b %kolumny
       ...
       H_lewo=m(i,j-1)+gap;
       H_gora=m(i-1,j)+gap;
       %jesli wartosci sa mniejsze od 0 - wpisac 0
       if H_skos<0
           H_skos=0;
       end
       if H_lewo<0
...
gaps=0;
f=true;
%dopasowanie sciezki i okreslenie sekwencji
while i>1 && f==true;
   while j>1 && f==true;
       if i~=1 && j~=1
       if str1(j-1)==str2(i-1) %preferowany kierunek po skosie
           ...
           H_gora=m(i,j-1);
           liczby=[H_skos,H_lewo,H_gora];
           H=max(liczby); %w przeciwnym razie wybranie maksymalnej
           sciezka=[sciezka,H];
           if (H==H_skos)
              i=i-1;
              j=j-1; 
       ...
       if((j==1&&i~=2) || (i==1&&j~=2)) %poruszanie sie wzdluz 1 wiersza lub 1 kolumny
          sciezka=[sciezka,0]; 
           if j==1
               i=i-1;
               dop_str1=[dop_str1,'–'];
...
dopasowanie=0;
%okreslenie procentu dopasowania
...
%zapisanie danych dla danej sciezki do struct
w1=fliplr(wiersze);
k1=fliplr(kolumny);
all(q).gaps=gaps;
